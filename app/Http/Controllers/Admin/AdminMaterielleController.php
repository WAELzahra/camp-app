...
use App\Http\Requests\Admin\AddMaterielPhotoRequest;
use App\Http\Requests\Admin\BulkMaterielStatusRequest;
use App\Http\Requests\Admin\UpdateAdminMaterielleRequest;
...
class AdminMaterielleController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('type')) {
            if ($request->type === 'rentable') {
                $query->where('is_rentable', true);
            }
            if ($request->type === 'sellable') {
                $query->where('is_sellable', true);
            }
            if ($request->type === 'both') {
                $query->where('is_rentable', true)->where('is_sellable', true);
            }
        }
    ...
    public function update(UpdateAdminMaterielleRequest $request, $id)
    {
        $mat = Materielles::findOrFail($id);
    ...
    public function addPhoto(AddMaterielPhotoRequest $request, $id)
    {
    ...
    public function bulkStatus(BulkMaterielStatusRequest $request)
    {
        $request->validated();
